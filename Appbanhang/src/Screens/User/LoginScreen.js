  ...
  const Login = async () => {
    try {
      if (!Email || !emailRegex.test(Email)) {
        ...
        const response = await axios.post(`${ipconfig}/Users/Login`, {
          Email,
          password,
        });
        if (response.status === 200) {
          await AsyncStorage.setItem('Email', response.data.user.Email);
          Alert.alert('Thông báo', 'Login thành công !');
      ...
      <ScrollView
        ...
        <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
          <View
            ...
            <Text style={styles.txt__login}>Đăng nhập </Text>
            ...
            <View>
              <TouchableOpacity style={styles.btn__Login} onPress={Login}>
                <Text>Đăng nhập</Text>
              </TouchableOpacity>
            </View>
